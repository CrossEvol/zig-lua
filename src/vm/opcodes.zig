...
pub const opcodes = [_]OpCodeStruct{
    ...
    opcode(0, 1, .OpArgU, .OpArgU, .IABC, "CALL    ", null), // R(A), ... ,R(A+C-2) := R(A)(R(A+1), ... ,R(A+B-1))
    opcode(0, 1, .OpArgU, .OpArgU, .IABC, "TAILCALL", null), // return R(A)(R(A+1), ... ,R(A+B-1))
    opcode(0, 0, .OpArgU, .OpArgN, .IABC, "RETURN  ", null), // return R(A), ... ,R(A+B-2)
